...
type InvertedIndexManager struct {
	indexes map[string]*model.InvertedIndex // 按名称存储多个索引
	mutex   sync.RWMutex
	loading map[string]*sync.Mutex // 正在加载的索引锁
}

// 全局单例实例
var invertedIndexManager = &InvertedIndexManager{
	indexes: make(map[string]*model.InvertedIndex),
	loading: make(map[string]*sync.Mutex),
}

func GetInvertedIndexManager() *InvertedIndexManager {
	return invertedIndexManager
}

func (fm *InvertedIndexManager) loadMutexForIndex(name string) *sync.Mutex {
	fm.mutex.Lock()
...
func (fm *InvertedIndexManager) LoadInvertedIndex(name string, index *model.InvertedIndex) error {
	// 为特定索引获取加载锁
